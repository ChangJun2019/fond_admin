...
@connect(({ posts, loading }) => ({
  allVideoStates: posts.allVideoStates,
  allTopics: posts.all_topics,
  loading: loading.models.posts
}))
@Form.create()
class VideoStatesManage extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: "posts/fetchAllVideoStates",
      payload: {}
    });
  };
  ...
  render() {
    ...
    const { loading, allVideoStates } = this.props;
    const { list,pagination} = allVideoStates;
     ...
     * @param item
     */
    const handleVideoStatesDelete = item => {
      Modal.confirm({
        title: "删除动态",
     ...
     * @param item
     */
    const handleVideoStatesGood = item => {
      const { dispatch } = this.props;
      dispatch({
     ...
     * @param item
     */
    const handeleVideoStatesTop = item => {
      const { dispatch } = this.props;
      dispatch({
    ...
    const moreAndVideoStates = (key, currentItem) => {
      if (key === "delete") {
        handleVideoStatesDelete(currentItem);
      } else if (key === "good") {
        handleVideoStatesGood(currentItem);
      } else {
        handeleVideoStatesTop(currentItem);
      }
    };
    ...
    function MoreBtn({data}) {
      return (
        <Dropdown
          overlay={
            <Menu onClick={({ key }) => moreAndVideoStates(key, data)}>
            ...
            renderItem={item => (
              <List.Item>
                <Card
                  ...
                  hoverable
                  cover={getVideoContent(item.videosrc)}
                  actions={[
...
export default VideoStatesManage;
